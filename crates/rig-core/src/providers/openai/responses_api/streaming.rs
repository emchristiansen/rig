...
use super::{
    CompletionResponse, FunctionCallArguments, GenericResponsesCompletionModel, Output,
    RESPONSES_TOLERATE_INCOMPLETE_KEY, ResponsesProviderExt, ToolStatus,
};
...
pub struct StreamingCompletionResponse {
    ...
    ///
    /// `F` and `U` added this field independently, with the same name, type,
    /// and purpose; the merge keeps one field carrying `F`'s fuller wording and
    /// `U`'s serde attributes.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    ///
    /// Independently convergent, as [`Self::status`].
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// [`StreamingCompletionChunk`] whose decode carries the fork's
/// arbitrary-precision routing.
///
/// `U` consolidated three of `F`'s four routed decode sites (the live SSE
/// loop, the buffered unary body, and `websocket::parse_server_event`) into
/// [`classify_responses_frame`], whose only decode point is
/// `wire::decode_known` — a bare `serde_json::from_str::<T>`. Routing there
/// would change every provider's decode, which D4 rejected outright.
///
/// The classifier is already generic over `T: DeserializeOwned` and uses `T`
/// nowhere but that one call, so substituting the decode is exactly a choice
/// of `T`: the tag scan, the duplicate-tag rejection, the non-object and
/// unknown-tag `Unknown` paths, and the `Corrupt`-on-typed-failure rule are
/// all untouched, and no shared code changes.
///
/// With the workaround off this is exactly `StreamingCompletionChunk`'s own
/// decode on the very same deserializer — no added layer, and error position
/// is preserved. With it on, the value is materialized at the decode root
/// before the typed decode, which is what sidesteps serde-rs/json#1157 and
/// which carries the duplicate-key relaxation D4 already documents and
/// accepts. `json_utils::from_str` is not reachable from here — it takes a
/// `&str`, and this is a generic `Deserializer`.
struct ApRoutedChunk(StreamingCompletionChunk);

impl<'de> Deserialize<'de> for ApRoutedChunk {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        {
            StreamingCompletionChunk::deserialize(deserializer).map(Self)
        }
...
pub(super) fn classify_responses_frame(data: &str) -> WireEvent<StreamingCompletionChunk> {
    wire::classify_tagged_frame::<ApRoutedChunk>(data, "type", is_known_responses_event_type)
        .map(|chunk| chunk.0)
}
...
/// The two independent stream policy axes.
///
/// `F` and `U` each added one axis to what was a single enum, and the merge
/// needs their product: a ChatGPT/Codex backend emitting completed tool calls
/// immediately can also be a caller that opted into tolerating a terminal
/// `response.incomplete`. A three-variant enum cannot spell that, and adding a
/// fourth variant would square again on the next axis, so the axes are carried
/// as what they are.
///
/// Only two constructors exist, and the split is deliberate. A boundary that
/// holds a [`completion::CompletionRequest`] must use
/// [`Self::for_request`], which reads the tolerance axis off that request
/// itself; a boundary that has no request uses [`Self::strict`]. There is no
/// free-form constructor, because one is what let two providers spell the
/// tolerance axis as a literal `false` and silently drop a caller's opt-in.
#[derive(Clone, Copy)]
pub(crate) struct ResponsesStreamOptions {
    /// Emit a tool call the provider delivered whole as soon as its
    /// `output_item.done` arrives, rather than buffering it to the terminal.
    immediate_tool_calls: bool,
    /// A terminal `response.incomplete` (e.g. a `max_output_tokens`
    /// truncation) is recorded and aggregated into the final response instead
    /// of erroring the stream. `response.failed` still errors. Lets a caller
    /// opt into surfacing truncation as a successful completion carrying
    /// `status`/`incomplete_details` evidence rather than a provider error.
    tolerate_incomplete: bool,
}

impl ResponsesStreamOptions {
    /// Policy for a seam that carries no request: no opt-in can have been
    /// expressed, so the tolerance axis is the strict default by construction.
    ///
    /// Reserved for genuinely request-less callers — item-chunk decoding, the
    /// conformance harness, and tests driving the accumulator directly. A
    /// provider's raw-stream boundary is never one of these.
    ...
    /// Policy for a request-less seam whose own contract already admits
    ...
    /// Policy for a request-carrying boundary: the caller supplies only the
    /// axis that is genuinely its own, and the tolerance axis is read off the
    /// request.
    ///
    /// This is the sole reader of [`RESPONSES_TOLERATE_INCOMPLETE_KEY`]. Every
    /// Responses raw-stream boundary routes through it, so a provider cannot
    /// express the tolerance axis independently — nor accidentally omit it,
    /// which is exactly how ChatGPT's and Copilot's routes came to ignore a
    /// caller's opt-in while OpenAI's honored it.
    ///
    /// Call this *before* the request is consumed by the wire conversion; the
    /// control key is dropped there, never reaching the provider body.
...
pub(crate) fn parse_sse_completion_body(
    ...
    for data in sse_data_frames(body) {
        // Upstream's shared framing helper; the fork's arbitrary-precision
        // decode routing (`json_utils::from_str`) survives in place here —
        // this is the one former routed site upstream did NOT consolidate
        // into `classify_responses_frame`.
...
impl RawChoiceAccumulator {
    ...
    pub(crate) fn record_response_chunk(
        ...
        // `F` captured status/incomplete_details here, ahead of the match,
        // because its `response.incomplete` fell through to `_ => Ok(())` and
        // nothing else would have recorded them. `U`'s terminal arm records the
        // same two values under the names `status`/`incomplete_details`, and
        // with the arm ordering below a tolerated incomplete now reaches it —
        // so the fork's separate `final_*` pair is the same variable recorded
        // twice, and is dropped rather than kept as a write-only duplicate.
        match kind {
            // MERGE ORDERING IS LOAD-BEARING. `F` reached this arm because its
            // predecessor arm matched `ResponseCompleted` alone; `U` widened
            // that arm to `ResponseCompleted | ResponseIncomplete`. Leaving the
            // guarded arm where `F` had it (after the widened arm) makes it
            // unreachable for `ResponseIncomplete`, silently dropping the
            // fork's strict-mode error. Placed first, the two policies compose:
            // an untolerated `response.incomplete` errors as at `F`, a tolerated
            // one falls through to `U`'s terminal arm and is aggregated.
            // `response.failed` errors either way — both sides agree.
            // Both sides agree: `response.failed` always errors.
            ResponseChunkKind::ResponseFailed => Err(
                crate::provider_response::completion_error_from_body(raw_event_data),
            ),
            // The fork's opt-in policy, unchanged: strict errors, tolerant
            // falls through to the terminal arm below.
            ResponseChunkKind::ResponseIncomplete if options.errors_on_terminal_chunk(&kind) => {
                Err(crate::provider_response::completion_error_from_body(
                    raw_event_data,
                ))
            }
            ...
            //
            // A tolerated `response.incomplete` reaches here, so `status` and
            // `incomplete_details` are recorded below and `finish()` surfaces
            // them on the final aggregated response — the fork's stated intent,
            // now served by upstream's own terminal arm.
    ...
    fn push_output_item_done(
        ...
        match item {
            Output::FunctionCall(func) => {
                ...
                // Item status supplies protocol confidence, not cause: it says
                ...
                let on_unparseable = match func.status {
                    ...
                    // This is the terminal normalization point, so a
                    // non-incomplete status is treated as asserted: malformed
                    // input is then a response defect, not a silent drop.
    ...
    if !replay_has_message_text {
        choice.extend(
            raw_response
                .output
                .iter()
                // Message content converts directly: the filter already
                // restricts this to message items, so routing them through the
                // fallible whole-item conversion would only add an error branch
                // that cannot occur.
                .filter_map(|item| match item {
                    Output::Message(message) => Some(message.content.clone()),
                    _ => None,
                })
                .flatten()
...
///
/// `options` is required rather than defaulted. A convenience overload that
/// hardcoded [`ResponsesStreamOptions::strict`] is what let Copilot's Responses
/// route silently drop the per-request incomplete-tolerance opt-in: the call
/// site read as a neutral "open the stream" and disclosed no policy at all.
/// Every caller now states its policy where a reader can see it.
    ...
    fn interpret(&mut self, event: ResponsesFrameEvent, out: &mut AdapterOutput<Self::Response>) {
        ...
        match event.chunk {
            ...
            StreamingCompletionChunk::Response(chunk) => {
                ...
                if let Err(error) =
                    self.accumulator
                        .record_response_chunk(kind, response, &event.raw, self.options)
                {
...
pub struct SummaryTextDeltaChunk {
    ...
    ///
    /// D5: `U`'s `#[serde(alias = "text")]` is deliberately NOT adopted. It
    /// lets a done-shaped payload parse silently as a delta; the fork's split
    /// shapes make that unrepresentable and are pinned by negative tests in
    /// both directions.
    pub delta: String,
}
    ...
    H: Clone + WasmCompatSend + 'static,
{
    ...
    pub async fn raw_stream(
        ...
        // Both axes at once: `U`'s provider-driven tool-call timing and `F`'s
        // per-call incomplete tolerance. Built before `create_provider_request`
        // consumes the request, which is where the control key is dropped.
        let options = ResponsesStreamOptions::for_request(
            &completion_request,
            Ext::EMITS_COMPLETE_TOOL_CALLS_IMMEDIATELY,
        );
        ...
        let stream = raw_stream_from_event_source(event_source, span, options);
        Ok(
...
mod tests {
    ...
    /// The provider-native terminal record, as `raw_stream` exposes it, with the
    /// per-call tolerate-incomplete opt-in set. It reads `raw_stream` rather than
    /// `stream` because the assertions need the Responses-native `status` and
    /// `incomplete_details`, which the normalized `StreamFinal` does not carry.
    async fn final_response_tolerating_incomplete(
        ...
        while let Some(item) = stream.next().await {
            if let RawStreamingChoice::FinalResponse(response) =
                item.expect("tolerated incomplete stream should not error")
            {
                return response;
            }
        }
    ...
    async fn response_incomplete_chunk_is_a_successful_terminal_with_mapped_finish_reason() {
        ...
        let client = openai::Client::builder()
            ...
            .expect("client should build");
        let model = client.completion_model("gpt-5.4");
        // The merged crate keeps `F`'s strict default: a terminal
        // `response.incomplete` errors unless the caller opts in per request.
        // `U` tolerated it unconditionally, so its test needs the opt-in `F`'s
        // own equivalents already set.
        let request = model
            .completion_request("hello")
            .additional_params(json!({
                super::RESPONSES_TOLERATE_INCOMPLETE_KEY: true
    ...
    async fn transport_error_flushes_delivered_tool_calls_before_the_error() {
        ...
        let mut stream = super::normalize_responses_stream(
            "openai",
            super::raw_stream_from_event_source(
                event_source,
                tracing::Span::none(),
    ...
    /// The mid-tool-call unparseable shape: `arguments_delta` frames
    /// stream a JSON prefix and the done item restates the same
    /// unparseable bytes under `status: incomplete`. Re-emitting the
    /// restatement as another raw delta put those bytes in the buffer
    /// TWICE — a delta-reassembling consumer rendered them twice and they
    /// were double-charged against the accumulation bound. Fragments seen →
    /// the buffer already holds the bytes; only a fragment-less done item
    /// (a pure replay of the restatement) still routes its raw string
    /// through the buffer at all (#2258 P3).
    #[test]
...
"
        ...
        let choices = raw_choices_from_sse_body(&body, ResponsesUsage::new())
            .expect("the unparseable shape must decode");
    ...
    /// The pure-replay half of the same policy: an unparseable restatement
    /// with NO preceding fragments must still reach the buffer (else the
    /// bytes never arrive and the status-derived unparseable-arguments
    /// policy has nothing to judge).
    #[test]
...
"
        ...
        let choices = raw_choices_from_sse_body(&body, ResponsesUsage::new())
            .expect("the replayed unparseable shape must decode");
    ...
    /// The first error the normalized stream surfaces, after draining whatever
    /// precedes it.
    ///
    /// Distinct from `first_error_from_event`, which requires the error to be
    /// the very first item: an unparseable restatement is routed to the buffer
    /// as a raw delta *before* the assembler judges it, so the error is not
    /// first.
    async fn first_error_after_draining(event: serde_json::Value) -> CompletionError {
        let client = openai::Client::builder()
            .http_client(MockStreamingClient {
                sse_bytes: sse_bytes_from_json_events(&[event]),
            })
    ...
    /// The `Completed` half of the status policy: a call the provider declares
    /// finished, whose restated arguments do not parse, is a response defect.
    ///
    /// Dropping it would silently lose a call the provider asserted; the error
    /// names the tool so an operator can find it.
    #[tokio::test]
    async fn a_malformed_asserted_call_surfaces_a_response_error() {
        let done = json!({
            "type": "response.output_item.done",
            "output_index": 0,
            "sequence_number": 1,
        ...
        let error = first_error_after_draining(done).await;
    ...
    /// Neither status may fabricate arguments. A restatement that does not parse
    /// and never reaches the buffer would hit the accumulator's parameterless
    /// branch and deliver `{}` — executing the call with arguments the provider
    /// never supplied.
    #[tokio::test]
    async fn a_malformed_restatement_never_fabricates_empty_arguments() {
        let done = |status: &str| {
            json!({
                "type": "response.output_item.done",
                "output_index": 0,
        ...
        // Not asserted by the provider: dropped, and no `{}` call is delivered.
        let client = openai::Client::builder()
            .http_client(MockStreamingClient {
                sse_bytes: sse_bytes_from_json_events(&[done("incomplete")]),
            })
            .api_key("test-key")
            .build()
            .expect("client should build");
        ...
        while let Some(item) = stream.next().await {
            if let StreamedAssistantContent::ToolCall { tool_call, .. } =
                item.expect("an unasserted call is dropped, not errored")
            {
        ...
        assert!(
            tool_calls.is_empty(),
            "an unparseable restatement must not become a call, least of all one \
             with fabricated `{{}}` arguments: {tool_calls:?}"
        );

        // Asserted by the provider: errors rather than delivering `{}`.
        let error = first_error_after_draining(done("completed")).await;
        assert!(
            matches!(&error, CompletionError::ResponseError(message)
...
"
    ...
    mod function_call_arguments {
        ...
        use crate::providers::openai::responses_api::{
            FunctionCallArguments, Output, ReconcileArgumentsError, UnparseableSide,
        };
        use serde_json::json;
        ...
        fn args_done_value(payload: &str) -> serde_json::Value {
            ...
            match chunk.data {
                ItemChunkKind::FunctionCallArgsDone(args) => match args.arguments {
                    FunctionCallArguments::Complete(value) => value,
                    other => panic!("expected complete arguments, got {other:?}"),
                },
        ...
        fn item_done_value(payload: &str) -> serde_json::Value {
            ...
            match chunk.data {
                ItemChunkKind::OutputItemDone(done) => match done.item {
                    Output::FunctionCall(call) => match call.arguments {
                        FunctionCallArguments::Complete(value) => value,
                        other => panic!("expected complete arguments, got {other:?}"),
                    },
        ...
        /// Arguments that cannot be parsed decode into `Unparseable` carrying
        /// their exact bytes, rather than failing the whole event.
        ///
        /// The bytes are the only record of what the provider actually sent, so
        /// losing the event would destroy the evidence a consumer needs; the
        /// decision about *executing* such a call belongs downstream, not here.
        ///
        /// Both events are checked: a decode that retained bytes on one side and
        /// failed closed on the other would reintroduce the asymmetry this
        /// shared type exists to remove.
        #[test]
        fn malformed_stringified_arguments_decode_as_unparseable_on_both_events() {
            const MALFORMED: &str = r#"{"command": "#;
            ...
            let args_chunk: ItemChunk = serde_json::from_str(&malformed_args_done)
                .expect("a malformed stringified argument must still decode");
            match args_chunk.data {
                ItemChunkKind::FunctionCallArgsDone(args) => match args.arguments {
                    FunctionCallArguments::Unparseable(raw) => assert_eq!(raw.as_str(), MALFORMED),
                    other => panic!("expected unparseable arguments, got {other:?}"),
                },
                other => panic!("expected FunctionCallArgsDone, got {other:?}"),
            }

            let item_chunk: ItemChunk = serde_json::from_str(&malformed_item_done)
                .expect("a malformed stringified argument must still decode inside an item");
            match item_chunk.data {
                ItemChunkKind::OutputItemDone(done) => match done.item {
                    Output::FunctionCall(call) => match call.arguments {
                        FunctionCallArguments::Unparseable(raw) => {
                            assert_eq!(raw.as_str(), MALFORMED)
                        }
                        other => panic!("expected unparseable arguments, got {other:?}"),
                    },
                    other => panic!("expected a function call item, got {other:?}"),
        ...
        /// The retained bytes survive a serialize/deserialize round trip
        /// verbatim: re-encoding them as JSON would destroy the evidence.
        #[test]
        fn unparseable_arguments_round_trip_verbatim() {
            const MALFORMED: &str = r#"{"command": "#;

            let original = serde_json::from_value::<FunctionCallArguments>(json!(MALFORMED))
                .expect("a malformed argument string decodes");
            let encoded = serde_json::to_value(&original).expect("arguments should serialize");

            assert_eq!(
                encoded,
                json!(MALFORMED),
                "the raw bytes are re-emitted exactly as they arrived"
            );
            let decoded = serde_json::from_value::<FunctionCallArguments>(encoded)
                .expect("the round trip decodes");
            assert_eq!(decoded, original);
        }

        /// The settled decode mapping, stated as one table.
        #[test]
        fn the_decode_mapping_covers_every_wire_spelling() {
            let decode = |value: serde_json::Value| {
                serde_json::from_value::<FunctionCallArguments>(value)
                    .expect("every spelling decodes")
            };

            // An empty or whitespace string is a parameterless invocation.
            assert_eq!(
                decode(json!("")),
                FunctionCallArguments::Complete(json!({}))
            );
            assert_eq!(
                decode(json!("   ")),
                FunctionCallArguments::Complete(json!({}))
            );
            // A string that parses carries its parsed value.
            assert_eq!(
                decode(json!(r#"{"x":1}"#)),
                FunctionCallArguments::Complete(json!({"x": 1}))
            );
            // A non-empty string that does not parse retains its bytes.
            assert!(matches!(
                decode(json!(r#"{"x":"#)),
                FunctionCallArguments::Unparseable(_)
            ));
            // A non-string payload is already parsed JSON: gateways emit the
            // object directly instead of the spec's stringified spelling.
            assert_eq!(
                decode(json!({"x": 1})),
                FunctionCallArguments::Complete(json!({"x": 1}))
            );
        ...
        /// Two observations of one call agree only when both parsed to the same
        /// value; an unparseable observation agrees with nothing, and names the
        /// side that failed.
        #[test]
        fn reconcile_settles_two_observations_of_one_call() {
            let complete = |value: serde_json::Value| FunctionCallArguments::Complete(value);
            let unparseable = || {
                serde_json::from_value::<FunctionCallArguments>(json!(r#"{"x":"#))
                    .expect("a malformed argument string decodes")
            };

            let left = complete(json!({"x": 1}));
            let agreeing = complete(json!({"x": 1}));
            let disagreeing = complete(json!({"x": 2}));
            let malformed = unparseable();
            let also_malformed = unparseable();
            let agreed = json!({"x": 1});

            assert_eq!(left.reconcile(&agreeing), Ok(&agreed));
            assert_eq!(
            ...
            assert_eq!(
                malformed.reconcile(&also_malformed),
