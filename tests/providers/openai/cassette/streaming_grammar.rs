    ...
    with_openai_cassette(
        ...
        |client| async move {
            ...
            let request = model
                ...
                .max_tokens(16)
                // The merged crate keeps the fork's strict default, so a
                // terminal `response.incomplete` is a per-request opt-in. This
                // contract's recording *is* that shape. The control key is
                // stripped before the wire body is built, so the cassette's
                // recorded request is unaffected.
                .additional_params(json!({
                    "reasoning": { "effort": "low" },
                    openai::responses_api::RESPONSES_TOLERATE_INCOMPLETE_KEY: true,
                }))
                .build();
            let run = drain_stream(model.stream(request).await.expect("stream should start")).await;
